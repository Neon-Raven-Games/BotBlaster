...
{
    private const string _DEFAULT_PATH = "Assets/AnimationGraph.json";
...
public static class GraphSerializer
{
    public static void GenerateTransitionsFromGraph(BaseGraph graph)
    {
        ...
        // decouple the transition from the view data
        foreach (var edgeView in graph.edges)
        {
            // transitions.Add(edgeView.transition);
        }

        // maybe we should construct the graph for runtime, but the UI code is
        // way too redundant to be used for runtime, tbh.
        // maybe we can streamline the data somehow, and reflect it into the graph
        // foreach (var transition in transitions)
        // {
        // lookupTable.AddTransition(transition.fromAnimation, transition.toAnimation, transition.blendDuration, transition.shouldBlend, transition.loopAfterTransition, transition.holdAfterTransition, transition.fallbackAnimation);
        // }
    }
