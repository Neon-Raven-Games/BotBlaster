...
namespace RNGNeeds
{
    ...
    public class ProbabilityList<T> : IProbabilityList
    {
        ...
        internal IProbabilityInfluenceProvider GetInfluenceProvider(int index) =>
            ProbabilityItems[index].InfluenceProvider;
        ...
        {
            return ItemCount > 1 && UnlockedItemsCount == 1 && ProbabilityItems[index].Locked == false &&
                   ProbabilityItems[index].BaseProbability > 0f;
        }
        ...
        public bool IsListInfluenced
        {
            get
            {
                foreach (var probabilityItem in ProbabilityItems)
                    if (probabilityItem.IsInfluencedItem)
                        return true;
                return false;
            }
        }
        ...
        public bool SetItemBaseProbability(int index, float baseProbability, bool normalize = true)
        {
            ...
            {
                RLogger.Log(
                    $"Index {index} out of range. There are {itemCount} items in this list. No item was changed.",
            ...
            {
                RLogger.Log($"Item at index {index} is Locked. Locked items cannot change probability.",
                    LogMessageType.Hint);
                return false;
            }
            ...
            {
                RLogger.Log(
                    $"Could not set Item {index} probability to {baseProbability:P5} because it is the only item in list. Total probability of list has to be 100%.",
                    LogMessageType.Hint);
                return false;
            }
            ...
            {
                RLogger.Log(
                    $"Could not set Item {index} probability to {baseProbability:P5} because it is the only unlocked item in list.",
                    LogMessageType.Hint);
                return false;
            }
            ...
            {
                RLogger.Log(
                    $"Could not set Item {index} probability to {baseProbability:P5} because there is only {availableProbability:P5} available probability in list. Some probability might be unavailable due to locked items or items with 0% probability.",
                    LogMessageType.Hint);
            ...
            for (var i = 0; i < itemCount; i++)
            {
                ...
                var probabilityItem = ProbabilityItems[i];
                if (probabilityItem.Locked) continue;
                probabilityItem.BaseProbability -=
                    (probabilityItem.BaseProbability / availableProbability) * deltaProbability;
        ...
        public float NormalizeProbabilities()
        {
            ...
            foreach (var item in ProbabilityItems)
            {
                if (item.Locked == false)
                {
                    ...
                    if (float.IsNaN(item.BaseProbability) || item.BaseProbability < .0000001f)
                        item.BaseProbability = 0f;
                }
        ...
        public ProbabilityItem<T> AddItem(T value, bool enabled = true, bool locked = false)
        {
            ...
            else if (unlockedCount > 0)
            {
                ...
                foreach (var pItem in ProbabilityItems)
                    if (pItem.Locked == false)
                        pItem.BaseProbability *= (1f - decreaseFactor);
            }
        ...
        {
            return ProbabilityItems.Contains(probabilityItem) &&
                   RemoveItemAtIndex(ProbabilityItems.IndexOf(probabilityItem), normalize);
        }
        ...
        public bool RemoveItemAtIndex(int index, bool normalize = true)
        {
            ...
            if (normalize)
            {
                ...
                {
                    RLogger.Log(
                        "Cannot remove the only unlocked item if it's probability is greater than zero. Other locked items cannot change probability, so the list cannot be normalized to sum up to 1 probability. Unlock at least one other item before removing this one or use parameter 'normalize = false' to remove item regardless of the resulting probability.",
                        LogMessageType.Hint);
                    return false;
                }
                ...
                {
                    RLogger.Log(
                        "Cannot remove item with 100% probability if there are other items in list. Items with 0% probability cannot be increased and normalized. Use parameter 'normalize = false' to remove item regardless of the resulting probability.",
                        LogMessageType.Hint);
                    return false;
                }
            }
        ...
        {
            if (TryGetProbabilityItem(index, out var probabilityItem))
                probabilityItem.InfluenceProvider = influenceProvider;
        }
        ...
        {
            if (TryGetProbabilityItem(index, out var probabilityItem))
                probabilityItem.InvertInfluence = invertInfluence;
        }
        ...
        public bool SetItemProperties(int index, float baseProbability, bool enabled, bool locked)
        {
            ...
            if (SetItemBaseProbability(index, baseProbability) && SetItemEnabled(index, enabled) &&
                SetItemLocked(index, locked)) return true;
